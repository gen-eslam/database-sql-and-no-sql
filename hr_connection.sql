...
--create index loc_state_city_idx on locations(city,state_province);
--create  index  emp_id_idx on emp_new(empno);
--desc user_ind_columns;
--select * from user_ind_columns;
--alter table dept_new add constraint test_con unique(dept_name);-->alter table dept_new modify (dept_name unique);
 --alter table dept_new drop constraint test_con;-->aleter table dept_new drop constraint test_con online;
--alter table emp_new drop primary key cascade;
--alter table dept_new disable constraint test_con;
--alter table emp_new disable primary key cascade;
--create table dept (dept_id number(3) constraint dept_pk primary key,dept_name varchar2(50));
--desc dept
--alter table dept add CONSTRAINT test_cn PRIMARY key (depitid);
--insert into dept values(110,'IT');
--select * from ALL_CONS_COLUMNS WHERE TABLE_NAME ='DEPT'  ;
--CREATE TABLE MY_DEPT(dept_id number ,dept_name varchar2(200));
...
--alter table my_dept DISABLE CONSTRAINT my_dept_id_pk;
--insert into my_dept values(1,'hr');
--alter table my_dept DISABLE NOVALIDATE CONSTRAINT my_dept_id_pk ;
--insert into my_dept values(2,'hr');
--select * from my_dept;
--delete from my_dept where dept_id=1 and dept_name ='hr';
--alter table my_dept enable CONSTRAINT my_dept_id_pk;
--alter table my_dept add CONSTRAINT dept_name_un UNIQUE(dept_name);
--insert into my_dept values(3,'hr');
--alter table my_dept drop constraint dept_name_un;
--desc user_constraints;
--select table_name,constraint_name,constraint_type,STATUS,validated from user_constraints where table_name like 'MY_DEPT';
--create global temporary table emp_temp on commit preserve rows as SELECT * FROM employees where department_id=90;
--select * from emp_temp;
--commit;
--select * from emp_temp;
--create global TEMPORARY TABLE my_temp_items (item_id number(5),item_name varchar2(50),item_description varchar2(100)) on commit PRESERVE rows;
--insert into my_temp_items values(110,'fares','sssssssssss');
--insert into my_temp_items values(110,'fares','zzzzzzzzzzzzzzz');
--commit;
--select * from my_temp_items;
--create or REPLACE DIRECTORY dir2 as 'D:\';
--CREATE TABLE ext_emp
--(first_name char (25), last_name char (25))
...
--(first_name char (25),last_name char (25))) LOCATION ('emp.txt'));
--select * from ext_emp;
--select last_name,hire_date,salary,manager_id from employees where (salary,manager_id) in (select salary,manager_id from employees where last_name ='Matos')and last_name !='Matos' ;
--select last_name,hire_date,salary from employees where (salary,department_id) in (select salary,department_id from employees where commission_pct is not null);
--select last_name,salary,job_id,department_id from employees out_table where salary>(select min(salary)from employees where department_id =out_table.department_id)order by department_id;
--select min(salary)from employees where department_id =employees.department_id;
-- select employee_id,last_name,salary ,department_id from employees out_table where exists(select * from employees where manager_id =out_table.employee_id) order by employee_id;
--select location_id,city,country_id from locations out_table where not exists(select * from departments where location_id =out_table.location_id) order by location_id;
-- select employee_id,last_name,salary ,department_id from employees out_table where not exists(select * from employees where manager_id =out_table.employee_id) order by employee_id;
--with dept_count as (select department_id,count(*) emp_count from employees GROUP BY department_id)
--select e.last_name,e.salary,d.emp_count, round(e.salary/d.emp_count) from employees e join dept_count d on(e.department_id= d.department_id)
--select department_name,sum(salary) from employees join departments using (department_id) GROUP by department_name having sum(salary)>(select sum(salary)*1/20 from employees)
--create table my_loc as(select location_id,country_id from locations);
--alter table my_loc add (country_name varchar2(200));
--select* from my_loc;
--update my_loc  set country_name = (select country_name from  countries where my_loc.country_id=countries.country_id );  
